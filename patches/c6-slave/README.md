...
| 0006 | `cmd-set-backlight-keyboard-channel.patch` | Adds a channel-select byte to `CMD_SET_BACKLIGHT` (repurposes `pad[0]`: 0 = display BL / LEDC ch 0, 1 = keyboard BL / LEDC ch 1; unknown values get `CMD_RESPONSE_FAIL`). Wire-compatible in both directions: pre-channel hosts zero the pad and keep addressing the display. Host counterpart is kernel patch 0024 (channel arg in `cmd_set_backlight()` + a second PWM channel + the DTS `kbd-backlight` node) — **protocol change: keep both sides in step**, an old slave interprets keyboard writes as display writes. |
