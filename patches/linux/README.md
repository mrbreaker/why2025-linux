...
| 0005 | `misc-esp32-c6-kick.patch` | Minimal driver to release the ESP32-C6 from reset at boot (P4 drives GPIO 13). Active-low; uses GPIOD_OUT_LOW to release. |
...
| 0007 | `mmc-dw_mmc-esp32p4-fixes.patch` | dw_mmc: restore multi-slot support via `snps,slot-id` DT property (mainline dropped it; we use slot 1 for SDIO to the C6) and a cached-descriptor-ring IDMAC fallback for ESP32-P4 cache-coherency oddities. |
